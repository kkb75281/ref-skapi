...
export default class Service {
    ...
    async registerSenderEmail(params: {
        ...
        let res = await skapi.util.request(
            this.admin_private_endpoint + "register-sender-email",
            Object.assign(
                { service: this.id, owner: this.owner },
    ...
    registerOpenIDLogger(params: {
        ...
        return skapi.util.request(
            ...
            Object.assign(
                { service: this.id, owner: this.owner },
                skapi.util.extractFormData(params).data || {}
            ),
            { auth: true }
        );
    }
    ...
    > {
        return skapi.createAccount(
            Object.assign({ service: this.id, owner: this.owner }, form),
            options
        );
    }
    ...
    async updateUserAttribute(
        form:
            | SubmitEvent
            | (UserAttributes & UserProfilePublicSettings & { user_id: string })
    ...
    async grantAccess(params: {
        ...
        return skapi.grantAccess(
            Object.assign({ service: this.id, owner: this.owner }, params)
        );
    }

    // get newsletter mail address
    async requestNewsletterSender(params: {
        group_numb: number;
    }): Promise<string> {
    ...
    checkCancel = () => {
        ...
        this.service.suspended =
            this.subscription?.cancel_at &&
            currTime >= this.subscription?.cancel_at * 1000;

        if (
            this.subscription?.canceled_at &&
            currTime >= this.subscription.canceled_at * 1000
        ) {
    ...
    async setServiceOption(opt: {
        ...
        try {
            let updated = await skapi.util.request(
                this.admin_private_endpoint + "service-opt",
                { service: this.id, owner: this.owner, opt },
                { auth: true }
            );
            return updated;
        } catch (err: any) {
    ...
    async getStorageInfo(): Promise<{
        ...
        wait.push(
            skapi.util
                .request(
                    this.record_private_endpoint + "storage-info",
                    { service: this.id, owner: this.owner },
                    { auth: true }
                )
                .then((r) => {
    ...
    async updateService(params: {
        ...
        if ("cors" in params) {
            ...
            let service_cors = (this.service.cors || "")
                .split(",")
                .map((c) => c.trim());
            if (
                cors.length === service_cors.length &&
                cors.every((v, i) => v === service_cors[i])
            ) {
                return this.service;
            }
    ...
    async getSubdomainInfo(
        ...
        stat:
            | "remove"
            | "change:(subdomain to be changed to)"
            | "active"
            | string;
        ["404"]?: string;
    } | null> {
        ...
        let exec = async (time = 1000) => {
            let info = await skapi.util.request(
                this.admin_private_endpoint + "subdomain-info",
                { service: this.id, owner: this.owner },
                { auth: true }
            );
            // devLog({ subInfo: info });
    ...
    async registerSubdomain(params: {
        ...
        if (this.service?.subdomain) {
            if (subdomain && this.service?.subdomain[0] === "*") {
                ...
                throw `Previous subdomain is in transit to "${this.service.subdomain.slice(
                    1
                )}".`;
            }
        }
    ...
    async set404(params: {
        ...
        return await skapi.util.request(
            this.admin_private_endpoint + "set-404",
            { service: this.id, owner: this.owner, page404: params.path },
            { auth: true }
        );
    }
    ...
    async uploadHostFiles(
        ...
        for (let [key, f] of (fileList as any).entries()) {
            ...
            let { fields = null, url } = await skapi.util.request(
                this.record_private_endpoint + "get-signed-url",
                signedParams,
                { auth: true }
            );
    ...
    async listHostDirectory(
        ...
        let dirList = await skapi.util.request(
            this.admin_private_endpoint + "host-directory",
            Object.assign({ service: this.id, owner: this.owner }, params),
            {
                fetchOptions: {
    ...
    async registerNewsletterGroup(params: {
        group: string;
        restriction: number;
    }): Promise<string> {
        ...
        return skapi.util.request(
            this.admin_private_endpoint + "register-newsletter-group",
            params,
            { auth: true }
        );
    }
    ...
    async newsletterGroupEndpoint() {
        let response = await skapi.util.request(
            this.admin_private_endpoint + "newsletter-group-endpoint",
            { service: this.id, owner: this.owner },
            { auth: true }
        );
    ...
    static async create(params: { name: string }) {
        ...
        if (regions?.[currentLocale]) {
            ...
            const calculateDistance = (locale: any, region: any) => {
                ...
                const Δφ =
                    ((region.latitude - locale.latitude) * Math.PI) / 180;
                const Δλ =
                    ((region.longitude - locale.longitude) * Math.PI) / 180;
        ...
        return new Service(service.service, service, [
            admin_private_endpoint,
            record_private_endpoint,
    ...
    static async load(id: string): Promise<Service> {
        ...
        if (typeof id === "string") {
            ...
            for (let region in service) {
                if (service[region][0]) {
                    let serviceClass = new Service(id, service[region][0], [
                        admin_private_endpoint,
                        record_private_endpoint,
                        admin_public_endpoint,
                    ]);
